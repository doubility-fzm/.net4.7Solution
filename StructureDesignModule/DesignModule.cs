...
namespace StructureDesignModule
{
    static class Constants
    {
        ...
        public const double spanOfBridge = 30;
        ...
        //加劲肋抗剪容许值取160MPa
        public const double stiffener_sheer_limits = 160;
    ...
    public class CrossSection
    {
        ...
        public double girder_upper_flange_width { get; set; }
        public double girder_upper_flange_thickness { get; set; }
        public double girder_lower_flange_width { get; set; }
        public double girder_lower_flange_thickness { get; set; }
        public double axis_moment_inertial_girder { get; set; }//转动刚度
        private double calculated_Moment { get; set; }//计算弯矩
        //计算纵向加劲肋的几何定位数据
        public (int nums,double thickness,double width,double start,double end) stiffener_info { get; set; }
        public List<double> stiffener_vertical_postion { get; set; }
        //新添加的成员属性，获取组合梁桥面板轮廓
        public double[] lateral_offset_dis { get; set; }
        public int vertical_offset_dis { get; set; }
        public int intermediate_beam_nums { get; set; }
        public double widthOfBridge { get; set; }

        public void calculate_stiffener_info(double girder_web_height,double girder_web_thickness,double start=0,double end=0)
        {
            double ratio = girder_web_height / girder_web_thickness;
            int nums=0;
            double thickness = girder_web_thickness;
            double width = girder_upper_flange_width / 2 - 0.03;
            if (ratio<=140)
            {
                nums = 0;
            }
            if(ratio>140&&ratio<=240)
            {
                nums = 1;
            }
            if(ratio>240)
            ...
            stiffener_info = (nums, thickness, width, start, end);
            stiffener_vertical_postion = new List<double>();
            //单位换算成mm
            switch (nums)
            {
                case 0:
                    stiffener_vertical_postion.Add(0);
                    break;
                case 1:
                    //取0.2倍的腹板高度为加劲肋的竖向偏移
                    stiffener_vertical_postion.Add(0.2 * girder_web_height * 1000);
                    break;
                case 2:
                    stiffener_vertical_postion.Add(0.14 * girder_web_height * 1000);
                    stiffener_vertical_postion.Add(0.36 * girder_web_height * 1000);
                    break;
                default:
                    break;
            }
        ...
        public void calculate_girder_parament(ref Technical_Demand t)
        {
            ...
            vertical_offset_dis = Convert.ToInt32(Math.Ceiling(th_plate / 10.0) * 10) + 150;
            widthOfBridge = t.Get_WidthOfBridge();
        }
        ...
        public void Write_Info2TXT()
        {
            ...
            sw.WriteLine("the bridge_plate_thickness is :{0}", vertical_offset_dis);
            ...
            sw.WriteLine("the longitudinal stiffener num is :{0}", stiffener_info.nums);
            for(int i=0;i<stiffener_vertical_postion.Count();i++)
            {
                if (stiffener_vertical_postion[i] == 0)
                    sw.WriteLine("there is no need add longitudinal stiffener");
                sw.WriteLine("the vertical offset distance of stiffener is :{0}",stiffener_vertical_postion[i]);
            }
            sw.WriteLine(str.PadRight(40, '-'));
            sw.Close();
        }
    ...
    public class longitudinal_lateral_connection
    {
        ...
        //单位采用mm
        public double stiffener_gap { get; set; }
        public int stiffener_nums { get; set; }
        public (int width,int height,int thickness) stiffener_info { get; set; }

        public void Calculate_stiffener_info(ref CrossSection t)
        {
            stiffener_gap = 950 * t.girder_web_thickness / Math.Sqrt(Constants.stiffener_sheer_limits);
            if(Convert.ToInt16(lateral_beam_gap)%Convert.ToInt16(stiffener_gap)>0)
            {
                stiffener_nums = Convert.ToInt16(lateral_beam_gap) / Convert.ToInt16(stiffener_gap) + 1;
            }
            else
            {
                stiffener_nums = Convert.ToInt16(lateral_beam_gap) / Convert.ToInt16(stiffener_gap);
            }
            stiffener_gap = lateral_beam_gap / stiffener_nums;
            int width=0, height, thickness;
            if(t.girder_upper_flange_width*1000/2-40>50)
            {
                width = Convert.ToInt16(t.girder_upper_flange_width * 1000 / 2 - 40);
            }
            height = Convert.ToInt16(t.girder_web_height * 1000 - 50);
            thickness = Convert.ToInt16(t.girder_web_thickness*1000);
            stiffener_info = (width, height, thickness);
        }
        public int Getintermediate_beam_nums()
        {
            return intermediate_beam_nums;
        }
        public double Getlateral_beam_gap()
        {
            return lateral_beam_gap;
        }

        private int typecount { get; set; }
        private double latera_height { get; set; }
        private int L_width { get; set; }
        private int L_thichness { get; set; }

        //获得计算参数
        public void get_parameters_truss(ref Technical_Demand t1, ref longitudinal_lateral_connection t2, ref CrossSection t3)
        {
            //计算横梁的数量
            ...
            stiffener_Z = 0;
            typecount = 0;
            double tmpheight = 90.0;
            double tmpthickness = 10.0;
            while (stiffener_Z == 0)
            {
                //空腹式试算Z的值首先采取L形角钢为2*L90*90*10之后逐渐增加角钢的尺寸
                double Area1 =2* (2 * tmpheight - 10) * tmpthickness / 1000000.0;
                latera_height = 0.75 * t3.girder_web_height;

                double TH = Math.Atan(latera_height*2 / (Math.Abs(t3.lateral_offset_dis[0] - t3.lateral_offset_dis[1])));
                axis_moment_inertial_lateral_beam = 4 * latera_height * latera_height / 9.0 * Area1 * (1.0 / (1 + 1 / (3 * Math.Cos(TH))));
                ...
                if (stiffener_Z <= 10)
                {
                    ...
                    if(typecount == 10)
                    {
                        tmpthickness = tmpthickness + 6;
                    }
                }
            }
            L_thichness = (Int32)tmpthickness;
            L_width = typecount * 10 + 90;
        }
        ...
        public void get_parameters(ref Technical_Demand t1, ref longitudinal_lateral_connection t2, ref CrossSection t3)
        {
            ...
            while (stiffener_Z == 0)
            {
                ...
                //空腹式试算Z的值首先采取L形角钢为L90*90*10之后逐渐增加角钢的尺寸
        ...
        public void Write_Info2TXT()
        {
            ...
            //sw.WriteLine("the lateral beam flange width is :{0}", lateral_flange_width);
            //sw.WriteLine("the lateral beam flange thickness is :{0}", lateral_flange_thickness);
            //sw.WriteLine("The lateral beam web height is :{0}",lateral_web_height);
            //sw.WriteLine("the lateral beam web thickness is :{0}", lateral_web_thickness);
            sw.WriteLine("the L size is :{0}*{1}*{2}", L_width, L_width, L_thichness);
            if (stiffener_Z > 10)
            {
